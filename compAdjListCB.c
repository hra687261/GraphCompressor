    ...
    if (v1 == window_nodes[c_w_ind] &&
      ...
      if (ca_len > 0) {
        ...
        if (ref > 0) {
          ...
          encoding_function(calcb->adj, u, nb_blocks);
            ...
            if (cpt < nb_blocks) {
              if (cv) {
                if (copy_list[i]) {
                  ...
                  encoding_function(calcb->adj, u, cpt1);
                  cpt++;
                  cpt0 = 1;
                  cv = !cv;
                }
                ...
                if (copy_list[i]) {
                  encoding_function(calcb->adj, u, cpt0);
                  cpt++;
                  cpt1 = 1;
                  cv = !cv;
                } else {
  ...
  uint64_t decoding_function(uint8_t *arr, uint64_t *s))
{
  *dlen = 0;
  ...
  if (u < calcb->cd[node + 1]) {
    ...
    if (ref > 0) {
      decode_CompAdjListCB(calcb, node - ref, dest, dlen,
        decoding_function);
      uint64_t nbb_sum = 0;
      uint8_t cv = 1;
      nb_blocks = decoding_function(calcb->adj, &u);
      while (cptblocks < nb_blocks - 1) {
        block_size = decoding_function(calcb->adj, &u);
        nbb_sum += block_size;
        if (cv) {
    ...
    if (u < calcb->cd[node + 1]) {
      dest[cptdecoded++] = inv_func(node, decoding_function(calcb->adj, &u));
      while (u < calcb->cd[node + 1]) {
        dest[cptdecoded] = dest[cptdecoded - 1] + decoding_function(calcb->adj, &u) + 1;
        cptdecoded++;
      }
    }
    *dlen = cptdecoded;
  ...
  uint64_t decoding_function(uint8_t *arr, uint64_t *s))
{
  ...
  *nbvals = 0;
  ...
  for (uint64_t i = s_file; i < e_file; i++) {
    ...
    decode_CompAdjListCB(calcb, v, dest, &dlen, decoding_function);
