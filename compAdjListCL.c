...
void free_CompAdjListCL(CompAdjListCL *a_l) {
  free(a_l->adj);
  free(a_l->cd);
  free(a_l);
}
  ...
  uint8_t mrc) 
{
  ...
  CompAdjListCL *alg = malloc(sizeof(CompAdjListCL));
  alg->n=0;
  alg->e=0;
  uint64_t v1, v2, 
      ...
      && ind < alg->e) 
    {
      ...
      if(ca_len > 0){
        ...
        if (ref > 0){
          ...
          //if(ra_it < ra_len)
          //  for(i=ra_it;i<ra_len;i++)
          //    copy_list[ra_pos[i]]=0;
        ...
        /*
        the left over values are stored in [nbv_refc .. ca_len[
        in increasing order
        */
  ...
  uint64_t decoding_function(uint8_t *arr, uint64_t *s)) 
{
  *dlen=0;
  ...
  if(u < alg->cd[node + 1]){
    ...
    if(ref>0) {
      decode_CompAdjListCL(alg,node-ref,dest,dlen,decoding_function);
      for(uint64_t i=0;i<*dlen;i++)
        if(read_nth_bit(alg->adj,u++))
          dest[cpt++]=dest[i];
    }

    if(u < alg->cd[node + 1]){
      dest[cpt++]=inv_func(node,decoding_function(alg->adj,&u));
      while(u < alg->cd[node + 1]){
        dest[cpt]=dest[cpt-1]+decoding_function(alg->adj,&u)+1;
        cpt++;
      }
    }
  *dlen=cpt;
    ...
    uint64_t decoding_function(uint8_t *arr,uint64_t *s))
{
  ...
  *nbvals=0;
  ...
  for(uint64_t i=s_file;i<e_file;i++){   
    ...
    seen[v]=1;
    dlen=0;
    decode_CompAdjListCL(alg,v,dest,&dlen,decoding_function);
